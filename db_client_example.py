...
WHERE r.duration_minutes = costs[i+1] - costs[i]
...
with driver.session() as session:
    ...
    for path in paths:
        ...
        for segment in path["segmentDetails"]:
            print(f"  - From: {segment['fromCity']}, To: {segment['toCity']}, Type: {segment['type']}, Duration: {segment['duration']} mins, Cost: {segment['cost']}, Route: {segment['routeName']}")
        print()
